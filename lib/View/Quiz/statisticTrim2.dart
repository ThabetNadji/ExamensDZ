...
class statisticTrim2 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Table(children: [
          TableRow(decoration: BoxDecoration(color: Colors.orange), children: [
            Center(
              child: Text(''),
            ),
            Text('الفصل الثاني',
            ...
            Center(
              ...
              child: FutureBuilder(
                future: _quizService.getExamNumberOfTrim(
                    userGlobal.getUserInfo().getUserID(), 'second_trim'), // x
                  ...
                  if (snapshot.hasData) {
                    ...
                    if (snapshot.data.toString() != '-1') {
                      ...
                      return FutureBuilder(
                        future: _quizService
                            .getNumberOfExamsInTrim('second_trim'), // y
            ...
            Center(
              child: FutureBuilder(
                future: _quizService.getScoreOfTrim(
                    userGlobal.getUserInfo().getUserID(), 'second_trim'),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    if (snapshot.data.toString() != '-1') {
                      return FutureBuilder(
